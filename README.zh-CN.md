...
│   ├── config.py            # 应用配置
│   ├── celery/              # Celery 应用与任务
│   ├── routers/             # API 路由模块（健康检查、视频、字幕、字典等）
│   ├── services/            # 业务服务层（任务管理、字典服务等）
│   ├── database/            # 数据库模型、会话与初始化
│   └── core/                # 核心业务逻辑
│       ├── asr/             # 语音识别（WhisperX 集成）
│       ├── translate/       # 翻译流水线
│       ├── split/           # 字幕分割
│       ├── analyze/         # 文本 / 日语分析
│       ├── llm/             # 大模型封装与提示词
│       ├── storage/         # 存储抽象（MinIO、workspace、缓存等）
│       └── utils/           # 工具函数
├── web/                      # Next.js 前端应用
│   ├── app/                 # Next.js app 路由
│   ├── components/          # React 组件（视频、字幕、字典等）
│   ├── lib/                 # 前端工具与 API 封装
│   └── locales/             # i18n 翻译文件
├── nginx/                    # Nginx 反向代理配置
│   ├── nginx.conf           # 主配置文件
│   └── README.md            # Nginx 使用说明与路由
├── models/                   # 本地 WhisperX 等模型文件
├── workspace/                # 输入 / 输出与缓存目录
├── scripts/                  # 辅助脚本（模型下载、测试脚本等）
├── tests/                    # 后端 API 自动化测试
├── docker-compose.yml        # 默认 Docker Compose
├── docker-compose.gpu.yml    # GPU 版 Compose（WhisperX 使用 GPU）
├── docker-compose.test.yml   # 测试环境 Compose（本地 / CI）
├── docker-compose.override.example.yml # 覆盖配置示例
...
- **Nginx 反向代理与路由说明**：`nginx/README.md`
- **后端 API 测试说明**：`tests/README.md`
