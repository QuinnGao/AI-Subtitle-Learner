...
class SubtitleSplitter:
    """字幕智能分割器（异步版本，适用于 FastAPI 环境）

    使用 asyncio 进行并发处理，使用 LLM 进行语义分段，支持缓存和规则降级。
    """
    ...
    async def split_subtitle(self, subtitle_data: Union[str, ASRData]) -> ASRData:
        """分割字幕(主入口)
        ...
        try:
            ...
            # 4. 并发处理（使用 asyncio.gather）
            processed_segments = await self._process_segments_async(asr_data_list)
    ...
    async def _process_segments_async(
        self, asr_data_list: List[ASRData]
    ) -> List[List[ASRDataSeg]]:
        """并发处理所有分段（异步版本）"""
        # 使用 asyncio.gather 并发执行所有任务
        tasks = [
            self._process_single_segment_async(asr_data) for asr_data in asr_data_list
        ]
        processed_segments = await asyncio.gather(*tasks)
        return processed_segments

    async def _process_single_segment_async(
        ...
        async with self.semaphore:
            try:
                return await self._process_by_llm_async(asr_data_part.segments)
            except Exception as e:
                logger.warning(f"LLM处理失败,使用规则降级: {str(e)}")
                # 规则处理是同步的，使用 asyncio.to_thread
                return await asyncio.to_thread(
                    self._process_by_rules, asr_data_part.segments
                )

    async def _process_by_llm_async(
        ...
        # LLM 调用是同步的，使用 asyncio.to_thread 在线程池中执行
        ...
        # 合并分段是同步的，也使用 asyncio.to_thread
        return await asyncio.to_thread(
            self._merge_segments_based_on_sentences, segments, sentences
        )
    ...
    def _split_long_segment(self, segments: List[ASRDataSeg]) -> List[ASRDataSeg]:
        ...
        while segments_to_process:
            ...
            if count_words(merged_text) <= max_word_count or n < RULE_MIN_SEGMENT_SIZE:
                # 保留原始字词分段的时间戳信息
                word_segments = current_segments.copy()
        ...
        for sentence in sentences:
            ...
            if best_ratio >= threshold and best_pos is not None:
                ...
                for group in seg_groups:
                    ...
                    # 保留原始字词分段的时间戳信息
                    word_segments = group.copy()  # 保存原始字词分段
