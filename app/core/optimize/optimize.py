...
class SubtitleOptimizer:
    ...
    async def optimize_subtitle(self, subtitle_data: Union[str, ASRData]) -> ASRData:
        """优化字幕
        ...
        try:
            ...
            # 并行优化（异步版本）
            optimized_dict = await self._parallel_optimize(chunks)
    ...
    async def _parallel_optimize(self, chunks: List[Dict[str, str]]) -> Dict[str, str]:
        """并行优化所有批次（异步版本）
        ...
        tasks = [self._optimize_chunk(chunk) for chunk in chunks]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        optimized_dict: Dict[str, str] = {}
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(f"优化批次失败：{str(result)}")
                optimized_dict.update(chunks[i])  # 失败时保留原文
            else:
                optimized_dict.update(result)

        return optimized_dict

    async def _optimize_chunk(self, subtitle_chunk: Dict[str, str]) -> Dict[str, str]:
        """优化单个字幕批次
        ...
        async with self.semaphore:
            try:
                result = await self.agent_loop(subtitle_chunk)
                ...
                if self.update_callback:
                    ...
                    # 回调函数可能是同步的，使用 asyncio.to_thread 执行
                    if asyncio.iscoroutinefunction(self.update_callback):
                        await self.update_callback(callback_data)
                    else:
                        await asyncio.to_thread(self.update_callback, callback_data)
    ...
    async def agent_loop(self, subtitle_chunk: Dict[str, str]) -> Dict[str, str]:
        """使用agent loop优化字幕
        ...
        for step in range(MAX_STEPS):
            ...
            if is_valid:
                # 对齐是同步的，使用 asyncio.to_thread
                return await asyncio.to_thread(
                    self._repair_subtitle, subtitle_chunk, result_dict
                )
