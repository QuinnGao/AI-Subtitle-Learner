...
class ASRDataSeg:
    def __init__(
        ...
        word_segments: Optional[List["ASRDataSeg"]] = None,
    ):
        self.text = text
        self.translated_text = translated_text
        self.start_time = start_time
        self.end_time = end_time
        self.tokens = tokens or []  # 词级别信息：text, furigana, romaji, type
        self.word_segments = (
            word_segments or []
        )  # 原始字词级别分段（用于保留字词时间戳）
...
class ASRData:
    ...
    def split_to_word_segments(self) -> "ASRData":
        ...
        for seg in self.segments:
            ...
            # 使用统一的多语言分词模式
            words_list = list[Match[str]](re.finditer(_WORD_SPLIT_PATTERN, text))
    ...
    def to_json(self) -> dict:
        """Convert to JSON format with tokens information"""
        result_json = []
            ...
            result_json.append(
                {
                    ...
                    "word_segments": [
                        ...
                        for word_seg in (segment.word_segments or [])
                    ],
                }
            )
        return result_json
    ...
    @staticmethod
    def from_json(json_data: list) -> "ASRData":
        """Create ASRData from JSON data

        Args:
            json_data: JSON 数据，必须是列表格式，包含 tokens 信息
        """
        segments = []

        for segment_data in json_data:
            start_time = int(segment_data.get("start_time", 0))  # 毫秒
            end_time = int(segment_data.get("end_time", 0))  # 毫秒
            text = segment_data.get("original_text", "")
            translated_text = segment_data.get("translation", "")
            # 处理 tokens，统一使用 start_time/end_time
            tokens = []
            for token in segment_data.get("tokens", []):
                token_data = token.copy()
                if "start" in token_data:
                    token_data["start_time"] = token_data["start"]
                    del token_data["start"]
                if "end" in token_data:
                    token_data["end_time"] = token_data["end"]
                    del token_data["end"]
                tokens.append(token_data)

            # 恢复原始字词分段的时间戳信息
            word_segments = []
            if "word_segments" in segment_data:
                for word_data in segment_data["word_segments"]:
                    word_seg = ASRDataSeg(
                        text=word_data.get("text", ""),
                        start_time=int(word_data.get("start_time", 0)),  # 毫秒
                        end_time=int(word_data.get("end_time", 0)),  # 毫秒
                    )
