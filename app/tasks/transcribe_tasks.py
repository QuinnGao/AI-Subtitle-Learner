...
@celery_app.task(
    name="app.tasks.transcribe.transcribe",
    bind=True,
    max_retries=3,
...
def transcribe_task(self, task_id: str, request_dict: dict):
    ...
    try:
        ...
        if video_task_id:
            ...
            if transcribe_task_obj and transcribe_task_obj.status == TaskStatus.COMPLETED:
                subtitle_file_path = transcribe_task_obj.output_path
                if subtitle_file_path and Path(subtitle_file_path).exists():
                    ...
                    _create_subtitle_task(video_task_id, task_id, subtitle_file_path)
...
def _create_subtitle_task(video_task_id: str, transcribe_task_id: str, subtitle_file_path: str):
    """创建字幕处理任务"""
    try:
        # 创建字幕处理任务
        ...
        logger.info(f"[Celery Task] 字幕处理任务已发送到队列: subtitle_task_id={subtitle_task_id}")
    except Exception as e:
        logger.error(
